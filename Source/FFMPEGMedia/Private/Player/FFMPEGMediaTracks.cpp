...
TArray<const AVCodec*>  FFFMPEGMediaTracks::FindDecoders(int codecId, bool hwaccell) {
    ...
    if (hwaccell) {
        ...
        for (const AVCodec* t_codec : tmp) {
            if (isHwAccel(t_codec)) {
                codecs.Add(t_codec);
            }
        }
    }
    ...
    if (candidates.Num() > 0) {
        if (!hwaccell) {
            for (const AVCodec* t_codec : candidates) {
                if (!isHwAccel(t_codec)) {
                    codecs.Add(t_codec);
                }
            }
...
int FFFMPEGMediaTracks::UploadTexture(FFMPEGFrame* vp, AVFrame *frame, struct SwsContext **img_convert_ctx) {
    ...
    *img_convert_ctx = sws_getCachedContext(*img_convert_ctx,
    ...
    FTimespan t_Duration = FTimespan::FromSeconds(vp->GetDuration());
                ...
                if (subpq.GetNumRemaining() > 0) {
                    ...
                    if (vp->GetPts() >= sp->GetPts() + ((float)sp->GetSub().start_display_time / 1000)) {
                        if (!sp->IsUploaded()) {
                            ...
                            FTimespan t_Duration = FTimespan::FromSeconds(sp->GetDuration());
                            ...
                            for (i = 0; i < (int)sp->GetSub().num_rects; i++) {
                                ...
                                if ( sub_rect->type == SUBTITLE_TEXT ||  sub_rect->type == SUBTITLE_ASS) {
                                    ...
                                    if (CaptionSample->Initialize(sub_rect->type == SUBTITLE_TEXT?sub_rect->text:sub_rect->ass, FVector2D(sub_rect->x, sub_rect->y), Time, t_Duration))
                                    {
...
int FFFMPEGMediaTracks::AudioDecodeFrame(FTimespan& Time, FTimespan& t_Duration) {
    ...
    t_Duration = FTimespan::FromSeconds(af->GetDuration());
...
void FFFMPEGMediaTracks::RenderAudio() {
    ...
    FTimespan Time = 0;
    FTimespan t_Duration = 0;

    audio_size = AudioDecodeFrame(Time, t_Duration);
    if (audio_size < 0) {
    ...
    if (CurrentState == EMediaState::Paused || CurrentState == EMediaState::Stopped) {
        ...
        if ( audioBuf != NULL ) {
            ...
            if (AudioSample->Initialize((uint8_t *)audioBuf, len1, targetAudio.NumChannels, targetAudio.SampleRate, Time, t_Duration))
            {
